...
vim.keymap.set('n', '<leader>sf', function()
	-- Search actual symbols (functions, classes, variables, ...) via LSP
	-- rather than raw text. Falls back to a fuzzy line search if no LSP
	-- client in the buffer supports document symbols.
	...
	if has_symbols then
		builtin.lsp_document_symbols({
			...
			-- Telescope only accepts an include-list, so enumerate everything else.
			symbols = {
...
-- TelescopeMatching (the highlight on matched characters in results) links to
-- the `Search` group by default, which is the same yellow-ish highlight used
-- for hlsearch matches. Give it its own subtle style instead.
